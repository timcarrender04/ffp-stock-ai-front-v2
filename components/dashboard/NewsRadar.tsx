    ...
    <div className="grid gap-6 lg:grid-cols-3">
      <Card className="border border-finance-green-25 bg-finance-surface-70/80 shadow-2xl backdrop-blur-xl lg:col-span-2">
        ...
        <CardBody className="flex flex-col gap-3">
          {events.length === 0 ? (
            <p className="text-sm text-zinc-400">No real-time events yet.</p>
          ) : (
            events.map((event) => (
              <div
                key={event.id}
      ...
      <div className="flex flex-col gap-6">
        <Card className="border border-finance-green-25 bg-finance-surface-70/80 shadow-2xl backdrop-blur-xl">
          <CardHeader className="flex items-center justify-between">
            ...
            <Icon
              className="text-xl text-finance-green"
          ...
          <CardBody className="flex flex-col gap-4">
            {articles.length === 0 ? (
              ...
              articles.slice(0, 4).map((article) => (
                <div
                  key={article.id}
                  className="flex gap-3 rounded-2xl border border-finance-green-30/60 bg-black/15 p-4"
                >
                  <div className="relative h-16 w-16 overflow-hidden rounded-xl border border-finance-green-30/60 bg-finance-surface-80">
                    {article.image_url ? (
                      <Image
