...
export function ActiveOrdersList({
  ...
  const [cancelingOrderId, setCancelingOrderId] = React.useState<string | null>(
    null,
  );
  ...
  const loadOrders = async () => {
    ...
    try {
      const response = await fetch(
        `/api/alpaca/orders?mode=${tradingMode}&status=open`,
      );
  ...
  const handleCancel = async (orderId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/alpaca/orders/${orderId}?mode=${tradingMode}`,
        {
          method: "DELETE",
        },
      );
      ...
      {orders.map((order) => (
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            {order.status !== "filled" &&
              ...
              order.status !== "rejected" && (
                <Button
                  color="danger"
                  isDisabled={cancelingOrderId === order.id}
                  isLoading={cancelingOrderId === order.id}
                  size="sm"
                  variant="flat"
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            {order.stop_price && (
              <div>
                <p className="text-zinc-400">Stop Price</p>
                <p className="text-white">
            ...
            <div>
              <p className="text-zinc-400">Time in Force</p>
              <p className="text-white">{order.time_in_force}</p>
            </div>
          </div>
          {order.submitted_at && (
